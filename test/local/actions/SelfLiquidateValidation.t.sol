...
contract SelfLiquidateValidationTest is BaseTest {
    function test_SelfLiquidate_validation() public {
        ...
        _sellCreditMarket(bob, candy, RESERVED_ID, 100e6, _maturity(30 days), false);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.LOAN_NOT_SELF_LIQUIDATABLE.selector, creditPositionId, 1.125e18, LoanStatus.ACTIVE
            )
        );
        size.selfLiquidate(SelfLiquidateParams({creditPositionId: creditPositionId}));
        vm.stopPrank();
